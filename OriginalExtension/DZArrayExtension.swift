...
public extension Array where Element: Equatable {
    ...
    func removeObjectsInArray(_ objects : [Element]){
        for obj in objects{
    ...
    - returns: true 代表包含
    */
    func containsObj(_ obj : Element)-> Bool{
        return self.contains(obj) == true
    }
    
    func findObj (_ obj: Element) -> Bool {
        return (index(of: obj) != nil) ? true : false
    }
...
public extension Array {
    ...
    ///
    /// - parameter block: matching block
    mutating func ck_performReject(_ block: @escaping (Element) -> Bool) {
...
extension Array{
    ...
    /// Array中是否包含 字符串
    ///
    /// - Parameter str: 待检测的字符串
    /// - Returns: 检测结果
    func containString(str:String) -> Bool{
    ...
    /// - Parameter str: 待检测的字符串
    /// - Returns: 检测结果
    func containInt(index:Int) -> Bool{
        if self.count == 0 { return false }
        for item in self {
            if "\(item)".toInt == index {
    ...
    mutating func addElements(_ new : Element){
        self.append(new)
    }
    ...
    func joinArrayBySeperator(_ separator:String) -> String {
    ...
    func cycle<U>(_ times: Int, callback: (Element) -> U) {
    ...
    ///
    /// :param callback function to call with the element
    func cycle<U>(_ callback: (Element) -> U) {
    ...
    func eachWithIndex(_ callback: (Int, Element) -> ()) -> [Element] {
...
//    }
    ...
    func each(_ when: (Element) -> Bool, callback: (Element) -> ()) -> [Element] {
    ...
    func every(_ callback: (Element) -> Bool) -> Bool {
    ...
    func fetch(_ index: Int, orElse: Element? = .none) -> Element! {
    ...
    func findIndex(_ callback: (Element) -> Bool) -> Int? {
    ...
    func findLastIndex(_ callback: (Element) -> Bool) -> Int? {
...
//    }
    ...
    /// :return Element at that index
    mutating func remove<T: Equatable>(_ value: T) -> T? {
...
extension Array{
    ...
    */
    func partitionBy <T: Equatable> (_ cond: (Element) -> T) -> [Array] {
        var result = [Array]()
        var lastValue: T? = nil
    ...
    */
    ...
    - returns: The set of elements for which call(element) is unique
    */
    func uniqueBy <T: Equatable> (_ call: (Element) -> (T)) -> [Element] {
        var result: [Element] = []
        var uniqueItems: [T] = []
...
//    }
    ...
    */
//    @available(*, unavailable, message: "use the '[U](range)' constructor") static func range <U: Comparable> (_ range: Range<U>) -> [U] {
//        return [U](range)
//        
//    }
    ...
    */
    subscript (interval: Range<Int>) -> Array {
        ...
        return self[rangeAsArray: (interval.lowerBound ..< interval.upperBound)]
    }
    ...
    */
    subscript (interval: ClosedRange<Int>) -> Array {
        return self[rangeAsArray: (interval.lowerBound ..< interval.upperBound + 1)]
    }
