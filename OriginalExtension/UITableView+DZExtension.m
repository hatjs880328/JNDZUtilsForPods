...
+(void)load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [self swizzleinitTableViewFrame];
    });
    
}


+(void)swizzleinitTableViewFrame{
    
    SEL originalSelector = @selector(initWithFrame:style:);
    SEL swizzledSelector = @selector(swizz_my_init:style:);
    
    DZ_SWIZZLE(originalSelector,swizzledSelector)
}


-(id)swizz_my_init:(CGRect)frame style:(UITableViewStyle)style {
    self.loading = YES;
    return [self swizz_my_init:frame style:style];
}
