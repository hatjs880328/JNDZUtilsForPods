...
public extension Dictionary where Value:Comparable {
    ...
    var valuesOrderedbyDescending:[Value] {
        return self.values.sorted().reversed()
    }
...
public extension Dictionary where Key:Comparable {
    ...
    var keysOrderedByDescending:[Key] {
        return self.keys.sorted().reversed()
    }
...
extension Dictionary {
    ...
    - returns:
    */
    func Swift2reflect(_ dict:[String: Any])->String{
...
public extension Dictionary {
    ...
    mutating func performSelect( _ block: (Key,Value) -> Bool){
    ...
    mutating func performReject( _ block: (Key,Value) -> Bool){
    ...
    func reject( _ block: (Key,Value) -> Bool) -> [Key:Value]
    {
    ...
    func select( _ block: (Element) -> Bool) -> [Key:Value]
    {
        var result: [Key:Value] = Dictionary()
        
        for (key,value) in self {
            if block(key:key,value:value)
            {
                result[key] = value
            }
        }
