...
static const BOOL networkCanUsedKey;
...
{
    // 注意，取出的是一个对象，不能直接返回
    id tmp = objc_getAssociatedObject(self, &networkCanUsedKey);
    NSNumber *number = tmp;
...
-(void)setNetworkCanUsed:(BOOL)networkCanUsed
{
    objc_setAssociatedObject(self, &networkCanUsedKey, @(networkCanUsed), OBJC_ASSOCIATION_ASSIGN);
}
