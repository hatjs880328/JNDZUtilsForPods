...
func abs(_ a:NSNumber,b:NSNumber,value:NSNumber)->Bool {
    let chat = a.doubleValue - b.doubleValue
    if chat > 0 {
        if chat > value.doubleValue {
            return true
        }else{
            return false
        }
...
class MathfunctionAnalyze {
    ...
    /**
     ...
     - returns: 最终结果
     */
    class func ayalyze(_ mathFunction:String)->Double {
        var MIDDLE_NUMBER:Double = 0
        var leftNumber:Double?
        var rightNumber:Double?
        for eachItem in mathFunction.components(separatedBy: ",") {
            //这里进行处理
            if leftNumber == nil {
    ...
    /**
     ...
     - returns: 结果
     */
    class func ayalyzeWow(_ mathFunction:[AnyObject])->Double {
        var MIDDLE_NUMBER:Double = 0
        var leftNumber:Double?
        var rightNumber:Double?
            ...
            for eachItem in mathFunction {
                if ifSep("\(eachItem)") {
                    ...
                    if leftNumber != nil && rightNumber != nil {
                        MIDDLE_NUMBER = septetorChange("\(eachItem)", leftNumber: leftNumber!, rightNumber: rightNumber!)
                        middle_array.append(MIDDLE_NUMBER as AnyObject)
                        leftNumber = nil
                        rightNumber = nil
                        continue
                    }else{
                        if leftNumber != nil {
                            middle_array.append(leftNumber! as AnyObject)
                        }
                        if rightNumber != nil {
                            middle_array.append(rightNumber! as AnyObject)
                        }
    ...
    /**
     ...
     - returns: 结果
     */
//    class func analyzeFunctionLastVersion(_ mathFunction:[AnyObject])->Double{
...
//        
//    
//        for (index:value) in mathFunction.enumerated() {
//            if index + 2 < mathFunction.count {
//                //正常处理，说明i后面还有至少俩对象
...
//    }
    ...
    /**
     ...
     - returns: true 是
     */
    class func ifSep(_ oper:String)->Bool {
    ...
    /**
     ...
     - returns: 结果
     */
    class func septetorChange(_ oper:String,leftNumber:Double,rightNumber:Double)->Double {
        switch oper {
